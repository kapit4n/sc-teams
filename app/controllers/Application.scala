...
class Application @Inject() (system: ActorSystem, config: Configuration) extends InjectedController {
  ...
  val googleAnalyticsCode = Some("Example")//config.getOptional[String]("gaCode")
  ...
  private def executeQuery(query: String, variables: Option[JsObject], operation: Option[String], tracing: Boolean) =
    QueryParser.parse(query) match {
      ...
      case Success(queryAst) =>
        Executor.execute(SchemaDefinition.StarWarsSchema, queryAst, new MatchTraitRepo,
            operationName = operation,
            variables = variables getOrElse Json.obj(),
            deferredResolver = DeferredResolver.fetchers(SchemaDefinition.characters),
            exceptionHandler = exceptionHandler,
            queryReducers = List(
              QueryReducer.rejectMaxDepth[MatchTraitRepo](15),
              QueryReducer.rejectComplexQueries[MatchTraitRepo](4000, (_, _) => TooComplexQueryError)),
